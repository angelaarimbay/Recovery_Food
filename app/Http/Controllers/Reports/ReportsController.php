        ...
        switch ($t->type) {
            ...
            case 'excel':
                ...
                foreach ($data as $key => $value) {
                    ...
                    $temp['outgoing_date'] = ($value1['outgoing_date'] ?  date("Y-m-d", strtotime($value[$key]['outgoing_date'])) : null);   array_push($dataitems, $temp);
                    }
                }
    ...
    public function MainInventoryReport(Request $t)
    {
        ...
        foreach (tbl_masterlistsupp::select('category')->whereRaw($where)->groupBy('category')->pluck('category') as $key1 => $value1) {
            ...
            foreach (tbl_masterlistsupp::where("category", $value1)->get() as $key => $value) {
                ...
                $a = clone $incoming_past; $temp['beginning_q'] = $a->sum('quantity');
                $a = clone $incoming_past; $temp['beginning_a'] = number_format($a->sum('amount'), 2);
