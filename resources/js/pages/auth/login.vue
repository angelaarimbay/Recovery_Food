    ...
    <v-row>
      <v-col xl="6" cols="12" class="m-auto">
        <v-card-text
          ...
          min-width="" 
        >
        <v-form ref="form"  @submit.prevent="login" @keydown="form.onKeydown($event)">
            ...
            <v-card-text>
              ...
              <v-row>
                <v-col cols="12" sm="12">
                  <v-text-field
                   :rules="rules.formRules" hide-details="auto"
                    label="Username"
                    outlined 
                    dense
                    v-model="form.email" :error-messages="form.errors.has('email') ? form.errors.errors.email[0] : ''"
                ...
                <v-col cols="12" sm="12">
                  <v-text-field
                    label="Password"
                    outlined :rules="rules.passwordRules"   
                    hide-details="auto"
                    dense v-model="form.password" :error-messages="form.errors.has('password') ? form.errors.errors.password[0] : ''"
              ...
              <v-row>
                <v-col cols="12">
                   <v-btn type="submit"  dark block color="red darken-2" :loading="form.busy" @click="login">Log In</v-btn> 
                </v-col>
              </v-row>
            </v-card-text>
          </v-form>
